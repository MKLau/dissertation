...
%%%%%INSERT GRAPHIC OF BIPARTITE NETWORK AND GENOTYPIC VARIANCE%%%%%
...
\section{How do ecological interaction networks evolve?}

The overarching question is how will species evolve in a community
context where species are inter-connected?

In a pairwise context...
%%%What is known about species evolution in a pairwise context?
%%%Lotka-Volterra
%%%Others...

Predictions from an individual pair context are unlikely to hold in
the context of the mulitple connections species may have in
communities. 

The structure of ecological interaction networks (e.g. food-webs and
mutualistic webs) are determined by the traits of individuals that
determine their distributions and how they interact with other
species. These traits are determined in part by genetics and both
biotic and abiotic factors. For example, a flowering plant is
pollinated by a set of animal species based on the traits of its
flowers and how they correspond to pollination traits in the animal
species. The length of the corrola is largely under genetic control,
but the amount of nectar present in a flower will be determined by
local environmental conditions, such as light availability or the
amount of visits by nectar feeding animals. 

The evolution of interactions requires that the traits underlying
interactions confer differential fitness to individuals within a
species. If this trait variation is heritable, then the differential
fitness can result in a shift in the frequency of traits underlying
interactions. Given this shift in interaction trait frequency, the
interaction within the context of the community will have evolved. It
is important to remember that phenotypic plasticity and behavioral
responses are two ecological (i.e. non-evolutionary) means for
individuals to shift traits and optimize their fitness. For example, a
predatory fish may shift its feeding behavior in response to shifting
abundances of prey.  

Defining an ecological network as the non-independence structure of a
group of species, the following are important criteria determining
how species' traits (both directly and indirectly linked to
interactions) and the connections among species change through time:

\textbf{Current Network Structure is Determined by:}
\begin{itemize}
\item physical traits
\item behavioral traits
\item resource availability (e.g. if enough of one prey is available
  why feed on another)
...
%One framework is to build off of the bipartite network literature,
%where we would predict that there would be structure in interactions
%within species creating sub-compartments within the larger structure
%of species. Interconnectedness among species is overlapping
%functionality. Lower interconnectedness creates unique interactions
%that could lead to separate evolutionary trajectories.
...
\section{Ecological Network Timeline}
