...
\item Bark Lichen Co-occurrence
  \begin{enumerate}
  \item Enter data from last Spring
  \item Analyze data
  \item Write
  \end{enumerate}
\item Rock Lichen Co-occurrence
  \begin{enumerate}
  \item Need to incorporate analyses into SCRL1
  \item Analyze co-occurrence patterns for SCRL2
  \end{enumerate}
\item Arthropod Co-occurrence
  \begin{enumerate}
...
To Do List
\begin{enumerate}
\item Move other project folders into dissertation/projects
\item Find lonsdorf paper and move into eff_com_div folder
\end{enumerate}
