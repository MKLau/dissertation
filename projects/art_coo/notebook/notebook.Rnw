...
\textbf{Tasks}
\begin{itemize}
\item Update script with addition of 'leaves' column
\item Deal with collapsing species
\item Analyses:
  \begin{enumerate}
  \item Overall network model for senescing
  \item Co-occurrence for senescing
  \item Co-occurrence by genotype paired
  \item 
  \end{enumerate}
\end{itemize}
...
\item Currently running co-occurrence modeling
\end{itemize}
