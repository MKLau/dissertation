...
\section{Tasks}
\begin{enumerate}
\item Add figure references
\item Double check figures and legends
\item Begin discussion
\end{enumerate}
...
<<>>=
key <- read.csv('/Users/Aeolus/projects/Sunset_Crater_Lichens/data/2012jul30/Key.csv')
x <- read.csv('/Users/Aeolus/projects/Sunset_Crater_Lichens/data/2012jul30/spp_env_combined.csv')
                                        #remove dead
x <- x[x$Live.Dead == 1,]
