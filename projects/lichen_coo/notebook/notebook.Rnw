...
\section{Tasks}
\begin{enumerate}
\item Finalize tree level analysis
\item Where would you send this? Two papers? PLoS One?
\item Look at notes on slides (in docs folder)
\end{enumerate}
...
\begin{enumerate}
\item SAP hold?
\item prfb letter of rec

\item wild: ses for tree level co-occurrence patterns
\item wild: ses responds to tree roughness

\end{enumerate}

notes:
\begin{itemize}
\item email cameron and rikke and lamit about interactions
\item biofilm on coral and other analogous
\item check ses calculations, starting with ses = 10
\item how does an ses of 10 compare to other communities?
\item resource co-occurrence pattern analysis
\item do tree level network modeling and compare that average to the
  stand level network structure
\item plant traits and community evolution
\item effective community diversity?
\item efd - sensitivity for missing parameters
\item think broad, how does this relate to other systems
\item start on gardens
\item think about information: a = -sum ti log tij/ti ?
\item correlate ses and richness
\item correlate with xgal
\item succession turn over
\item look at age
\end{itemize}


\section{meeting with tom 18sep2013}
...
\section{tasks}
\begin{itemize}
\item try networks plotted with nodes arrayed along an axis of
  environment (i.e. the points centered where their abundance is
  highest)
\item need to reconcile taxonomic differences
\end{itemize}
