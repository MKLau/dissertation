...
\item PB effects: 
  ...
  \item PB frequencies are different among genotypes (X2 test: 125.62,
    df=18, P=2.2e-16), frequencies of 1 and 2 PB per leaf are
    different among genotypes on live leaves (X2=6.701,P=0.0096 and
    X2=6.27,P=0.012), while 3 PB/leaf for SEN (X2=4.045,P=0.044)
  \end{itemize}
\item Co-occurrence patterns
  ...
  \end{itemize}
\item Nestedness
  \item R1 nestedness is only significant for Live leaves
  \item R00 and R0 nestedness are significant for both
  \item C0 nestedness is only signficant for Sen leaves
\end{itemize}
