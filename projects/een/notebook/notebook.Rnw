...
selection <- (0.00007924*2.511886^((1:8)-1))
plot(tree.pheno[,1]~tree.gpm[,2])
...
###Nestedness by genetic variance
...
plot(density(ex.split[[1]]$typ),main='',col=1,ylim=c(0,20));lines(density(ex.split[[2]]$typ),col=2)
                                        #selection
par(mfrow=c(1,2))
plot(rnd~jitter(selection),data=ex.split[[1]],col='red',ylab='Percent Nodes Removed')
points(deg~jitter(selection),data=ex.split[[1]],col='green')
points(typ~jitter(selection),data=ex.split[[1]],col='blue')
plot(rnd~jitter(selection),data=ex.split[[2]],col='red',ylim=c(0.85,1),ylab='Percent Nodes Removed')
points(deg~jitter(selection),data=ex.split[[2]],col='green')
points(typ~jitter(selection),data=ex.split[[2]],col='blue')
