...
\section{Current Results and Analyses Left to Finish}

\begin{itemize}
\item Garden
  \begin{itemize}
  \item Height doesn't influence composition (PERM: F=1.88,P=0.154,R2=0.017,n=999)
  \item NMDS: stress = 0.1727378, r2 = 0.894879
  \item Roughness is highly heritable (REML: X2=9.214,P=0.0024,H2C=0.36)
  \item Roughness predicts relativized community composition (PERM: R2=0.04,
    P=0.04)
  \item Stand level co-occurrence patterns significant using the
    community matrix (SES=-11.32535,P<<0.0001) (stand level network figure), but
    genotype doesn't predict SES (REML:  X2<<0.0001,P=0.996)
  \item genotype-species networks are nested (r1: z=-3.496,P=0.001)
  \end{itemize}
\item Wild
  \begin{itemize}
  \item Height doesn't influence composition (PERM: F=0.51,P=0.712,R2=0.019,n=999)
  \item Significant co-occurrence patterns at the stand scale. 
  \item Tree level co-occurrence patterns influenced by roughness. 
  \item Unipartite network.
  \item Nestedness significant.
  \end{itemize}
\end{itemize}
...
\begin{itemize}
\item genotype effects on richness, diversity and composition
\item genotype effects on roughness
\item roughness correlation with richness, diversity, eveness and compostion
\item co-occurrence patterns (stand, tree, individual = genotype)
\item nestedness (stand, tree, genotype: r00, r0, c0, r1)
\end{itemize}


\section{24 Mar 2014}

Finishing ONC (lcn_onc.R) and doing a quick view of Pit (lcn_pit.R).
...
Co-Occurrence PerMANOVA tests if observations of positive or negative
co-occurrence patterns vary significantly with respect to a
categorical or continuous predictor variable.

\begin{enumerate}
\item Co-occurrence counts (co)
\item Checker counts (ch)
\item Ratio of co to ch
\end{enumerate}
...
\item Wild
  \begin{enumerate}
  \item Height composition = not related
  \item Overall Co-occurrence Patterns = statistic    1.3556 -2.9207 0.0005999
  \item Nestedness = 8.6663 -2.5715 0.0018
  \item SES ~ Roughness = p=0.0502, r2 = 0.2233
  \item co ~ Roughness = p=0.5975
  \item ch ~ Roughness = p=0.1058
  \item ch ~ co ~ SES = all negatively correlated
  \item Significant Co-occurrence network
  \item Get pos and neg co-occurrence
  \end{enumerate}
\item ONC
  \begin{enumerate}
  \item Height composition
  \item Overall Co-occurrence Patterns
  \item Nestedness, Modularity, Centrality
  \item SES ~ Genotype or RFLP distance
  \item co ~ Genotype or RFLP distance
  \item ch ~ Genotype or RFLP distance
  \item ch ~ co ~ SES
  \end{enumerate}
\item Pit
  \begin{enumerate}
...
\begin{itemize}
\item Start integrating comments from lab meeting into manuscript
\item Ask Cameron and Rikke for input on lichen-lichen interactions

\end{itemize}

\section{28 Oct 2013}

\textbf{Wild Stand Analysis Summary}
- Network modeling of co-occurrence patterns at the stand scale
revealed statistically significant structure (Fig1. Stand Network
Model)
- Abundance not correlated with centrality (P = 0.795)
- Trees varied in co-occurrence patterns (Table 1. SES and P-values)
- Geographic distance did not explain co-occurrence patterns (Mantel,
P = 0.78) or bark roughness (P = 0.80)
- Bark roughness significantly affected co-occurrence patters (r2 =
0.42, P = 0.0127, Fig2. Scatterplot SES)
- Tree age did not directly affect co-occurrence patterns (P = 0.098)
but did indirectly influence co-occurrence patterns via a significant
affect on roughness (SEM: chi2=0.38, P = 0.54, Fig. 3 ?)


\begin{enumerate}
\item enter wild stand tree data 
\item send tree data to lamit
\item check data
\item Final analyses
...
\begin{enumerate}
\item Microsite
\item Establishment (i.e. tree age)
\item Tree traits
\end{enumerate}

\section{22 Oct 2013}

Got wild stand tree data from Lamit. Use data/unitau_trees/FAX_20131022_1382474004_17.pdf.

\begin{itemize}
\item Bayesian stats?
\item New Paper Idea: Wild stand and tree level analyses
\item New Paper Idea: Mantel of garden and wild or QAP (Common garden similar to wild)
\item New Paper Idea: Replicated genotypes in common garden
  \begin{itemize}
  \item Genetic distance = email nash
  \item email Andy about next gen sequencing
  \item Non-bark effects
  \end{itemize}
\item genotype level co-occurrence analyses and network models
\item tannins
\item Check entry error
\item SEM?
\item Genetic similarity?
\end{itemize}

ONC preliminary analyses:

\begin{itemize}
\item Stand level network structure is significant (see SES =
  -44.8416, P < 0.001)
\item Roughness ~ genotype P = 0.000001
\item SES ~ genotype P = 0.318
\item Degree ~ genotype P = 0.053
\item Degree ~ rougness P = 0.0661
\item Centrality ~ genotype P = 0.0783
\item Centrality ~ roughness P = 0.185
\item Network similarity ~ genotype = 0.273
\end{itemize}


\section{21 Oct 2013}

\textbf{Preliminary Analysis of ONC Co-occurrence Patterns}

For next set of analyses, re-do ses and make non-significant ses
values zero.

\begin{itemize}
\item Removed RL6 due to low sample size and weird tree
\item Also removed N1.31 as it is also an odd tree for 1008
\iem Relativized composition correlates with genotype and roughness
\item SES doesn't correlate with genotype or roughness
\item Network similarity doesn't correlated with genotype or roughness
\item Network degree does correlated with genotype (P = 0.053) and
  marginally with roughness (P = 0.066)
\item Network centrality marginally correlated with genotype (P =
  0.074) but not roughness
\end{itemize}

Stand and tree SES calcuations are now handled by the src/onc_ses.R
script for ease of access to server. Output data are in
data/onc_stand_null.Rdata and data/onc_tree_ses.Rdata.

\textbf{Note on Pit data collection}
Pit data collection in 2012 was done to extend the dataset collected
in 2011 by collection more trees and the upper quadrats. These have
not been entered. Pit data from 2011 only has 45.55 quadrat.


\section{18 Oct 2013}

Neither Pit nor ONC show an effect of genotype or roughness on composition.

\textbf{ONC Analyses}
\begin{enumerate}
\item Stand level SES = -44.86902, P < 0.001
\item Tree
  \begin{enumerate}
  \item Upper differs from lower, but this might not matter for the
    co-occurrence patterns, only for the scale at which they manifest
    within a tree
  \item Limit to 2011 only (2011 has all of 2010 trees plus more)
  \item 
  \end{enumerate}
\end{enumerate}
...
\textbf{Wild Results}
\begin{enumerate}
\item Lec and Phy combined
\item Height doesn't influence comp (P = 0.717)
\item Stand SES = -20.5079, P $<<< 0.0002$, See ``results/figs/Wild Stand-level CoNet.png''
\item C-scores are not significantly related to roughness (P = 0.169)
\item SES varies (see ``wild stand-level cscore densities), but generally negative (Mean = -4.2521, SE = 0.9611965)
\item SES negatively correlated with Roughness (Percent Rough Bark) (P
  = 0.0127, R2 = 0.417)
\item Age predicts roughness (R2 = 0.2959, P = 0.044) but not SES (P =0.0983)
\item SEM shows that age prdicts roughness predicts SES
\end{enumerate}
